...
class BirdClefModelBase(pl.LightningModule):
    def __init__(self, cfg, stage):
        ...
        self.birds = [f'bird{i+1}' for i in range(num_classes)]
        self.stage = stage
        self.cfg = cfg
        ...
        if self.loss == 'ce':
            self.loss_function = nn.CrossEntropyLoss(label_smoothing=self.cfg.label_smoothing,reduction='none')
        elif self.loss == 'bce':
            self.loss_function = nn.BCEWithLogitsLoss(reduction='none')
        else:
            raise NotImplementedError
        self.mixup = Mixup(mix_beta=self.cfg.mix_beta,mixup_prob=self.cfg.mixup_prob, mixup_double=self.cfg.mixup_double)
        self.mixup2 = Mixup2(mix_beta=self.cfg.mix_beta2,mixup2_prob=self.cfg.mixup2_prob)
        self.ema = None

        self.audio_transforms = Compose([
            #AddColoredNoise(p=0.5),
            PitchShift(min_transpose_semitones=-4,max_transpose_semitones=4,sample_rate=self.cfg.SR,p=0.4),
            Shift(min_shift=-0.5,max_shift=0.5,p=0.4),
        ])

        self.time_mask_transform = torchaudio.transforms.TimeMasking(time_mask_param=60,iid_masks=True,p=0.5)
        self.freq_mask_transform = torchaudio.transforms.FrequencyMasking(freq_mask_param=24,iid_masks=True)

        #window = torch.hann_window(window_length = 2048, device = self.cfg.DEVICE)
        self.melspec_transform = torchaudio.transforms.MelSpectrogram(sample_rate=self.cfg.SR, hop_length=self.cfg.hop_length, n_mels=self.cfg.n_mels, f_min=self.cfg.f_min, f_max=self.cfg.f_max, n_fft=self.cfg.n_fft, center=True, pad_mode='constant',norm='slaney',onesided=True,mel_scale='slaney')
    ...
    def lower_upper_freq(self,images):
        ...
        pink_noise = torch.from_numpy(np.array([np.concatenate((1-np.arange(r)*x/r,np.zeros(self.cfg.n_mels-r)-x+1))]).T).float().to(self.cfg.DEVICE)
    ...
    def validation_epoch_end(self,outputs):
          ...
          if self.current_epoch>-1:
            ...
            self.log('validation C-MAP score pad e',avg_score2, on_step=False, on_epoch=True, logger=True, prog_bar=True)
...
class BirdClefModelSED(BirdClefModelBase):
    def __init__(self, cfg, stage):
        ...
        base_model = timm.create_model(
           cfg.model_name, pretrained=True, in_chans=self.in_chans, drop_path_rate=0.2,drop_rate=0.5)
        ...
        self.att_block = AttBlockV2(
            in_features, self.num_classes, activation="sigmoid")
    ...
    def forward(self,batch):
        ...
        if not self.training:
            ...
            segmentwise_logit = segmentwise_logit.reshape((bs,parts,seg_num,-1)).max(dim=1).values
            framewise_logit = framewise_logit.reshape((bs,parts,fram_num,-1)).max(dim=1).values

        loss = 0.5*self.loss_function(torch.logit(clipwise_output), y) + 0.5*self.loss_function(segmentwise_logit.max(1)[0], y)
...
class BirdClefModelCNN(BirdClefModelBase):
    def __init__(self, cfg, stage):
        super().__init__(cfg, stage)

        self.backbone = timm.create_model(cfg.model_name, pretrained=True,in_chans=self.in_chans,num_classes=0,global_pool="", drop_path_rate=0.2,drop_rate=0.5)
    ...
    def forward(self,batch):
        ...
        x = x[:, :, 0, 0]
        if self.training:
          logits = torch.mean(torch.stack([self.head(self.big_dropout(x)) for _ in range(5)],dim=0),dim=0)
        else:
          logits = self.head(x)
